...
![**Fig 1.2**: Four Step Model](../graphics/intro/Four_Step_Model.png){width=75%}
...
We then went into NHTS data to pull survey data about the existing Mode Share in OKC. We divided the modes into three categories: HOV, SOV, and Transit. 
...
|SOV|Car, SUV/VAN, Pickup Truck, Rental Car, Rideshare/Taxi|
|HOV| Same as Car, but occupancy >1|
...
1. Using a Nested Logit when possible and 
2. Using models that we had all values for

For Home Based Work Trips we Chose

We chose the following models

|Trip Type| Model Chosen|Assumptions of Model|
|---|---|---|
|Home Based Work|**Model G**|Nested Logit for more than 1 million, Does not included motorized, has submodes for auto but none for transit|
|Home Based Other| **Model G**|For more than a million, Not a nested logit, does not include motorized or transit submodes, does include auto submodes|
|Non Home Based|**Model G**|More than a million, Not a nested logit, does not include motorized or transit submodes, does include auto submodes|

#### Calculating Utility by Applying a Mode-Choice Model
The next step we needed to do was to calculate utility. To figure out how much someone's utility of a trip depends on characteristics of that trip. For example how long someone has to wait for a bus decreases their utility of transit and how long someone has to drive decreases their utility of driving.The NCHRP models give us coeficients (how different aspects such as waiting time influence utility) but they don't provide mode-specific constants which would tell us how the utilities of modes relate to each other. To estimate the utility of each mode we will start by using the log-odds to generate constants, and then return and adjust them when we are calibrating our model.   

Once we have utility for each trip we can calculate probability of each mode by trip type. This ultimately lets us calculate number of trips by mode. 


#### Calculate Regional Mode Shares
Now that we have number of trips per mode by each GEOID we can use our flow data generated through our gravity model in last section to estimate mode shares across OKC. 


When we compared this to NHTS data we can see that the numbers are close, but off. We then altered the utility constants that we generated through calculating log-odds for each mode share until our model matched NHTS data.
...
The other things we needed for this step were information about the capacity if each link (road) in our network and free flow travel times. We calculated capacity by assuming that capacity is a function of the number of lanes and the speed of the road. We assumed that all roads that did not have lanes in the open street map dataset were 2 lane roads. And we assumed that the capacity of the road would be 1,800 vehicles per hour multiplied by the speed of the road divided by sixty. 
