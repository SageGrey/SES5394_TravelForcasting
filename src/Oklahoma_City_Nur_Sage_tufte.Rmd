...
For our scenario, we model what would happen if Oklahoma ceded to the demands of the Land Back Movement and returned sovereignty and control of a considerable amount of land that was stolen from the Creek, Seminole, and Kickapoo communities in the seven [land runs that occurred between 1889 and 1906](https://nationalcowboymuseum.org/explore/rushes-statehood-oklahoma-land-runs/) (land was stolen from many other nations in Oklahoma but are outside our study area). 

We will test a scenario where after reclaiming sovereignty, native communities choose to build mixed-use with high-density affordable housing. This is inspired by the recent [Senakw development](https://www.theguardian.com/cities/2020/jan/03/the-bold-new-plan-for-an-indigenous-led-development-in-vancouver) in Vancouver where the Canadian government repatriated land to the Squamish nation which decided to build a district of dense affordable housing that was well outside of the previous zoning regulations. We will alter our model by dramatically increasing the population and employment numbers for TAZs that we identify as being returned to native communities. For the sake of this model, we decided to have this increase in population be made up of people moving into the study area from other parts of the country. This decision was informed in part by the observation that Oklahoma City is projected to have less severe drought than other regions of the Southwest due to climate change and could see an increase of domestic populations moving there as well as international climate refugees. 


```{r climate-risk-map, fig.margin=T, fig.cap="Assessment of Climate Risks for OKC through 2050 by ClimateCheck.com  ", cache=T}
knitr::include_graphics("../graphics/scenario/OKC_ClimateChangeRisk.png")
```
...
```{r matching-map, fig.margin=T, fig.cap="Matching Maps of Land Seized to Census Tracts", cache=T}
knitr::include_graphics("../graphics/scenario/closeup_georeference.jpeg")
```
...
### Data
We did not use any additional data sources for this scenario.


### Evaluation Criteria
There were three things we wanted to look at in this scenario: Travel Flows, Total VMT, and Congestion. 

#### Travel Flows
To begin, we wanted to see the impact this scenario would have on the travel flows between counties. In our current model, the overwhelming majority of travel for all three trip types originates in Cleveland county and goes to Oklahoma County. Our hypothesis is that in this scenario there will be a more even distribution of trips origin and destination across the 7 counties in our study area.

#### Congestion
One of the things we wanted to understand with this scenario is how well the current road network infrastructure could handle a dramatic increase in population. To look at this we wanted to look at the link vehicle volumes (the output from the trip assignment phase of our model) to see how much link vehicle volumes exceed the link capacities. Since there is no public transit routes that reach our study area, we will only be looking at vehicles. 

#### Total Vehicle Miles Traveled (VMT)
We also wanted to understand the impact this would have on total Vehicle Miles Traveled. In this scenario we would be essentially proposing a situation where the OKC metro area is no longer organized around a central point (downtown OKC), instead the land back scenario would create a more polycentric distribution. This would make the region follow a logic closer to the Releigh-Durham-Cary research triangle, Minneapolis-St. Paul or Dallas-Fort Worth. The VMT will go up with more population, but will it go up in direct proportion to population or would it be influenced by other factors such as centralized vs poly-centric development. 
...
What we can see from these diagrams depicting flows is that our scenario, as expected, increases the flows to Lincoln county. 
...
Looking at our chord diagram, we can also see that there are now significantly more trips going to and from Lincoln County. What's interesting to note is that this includes many other trips that are being diverted from other destinations, not just new trips. In our original model for Home Based Work trips there were 408,172 trips that left Cleveland County for Oklahoma County and no trips that left Cleveland for Lincoln County. In our scenario model, there are only 311,942 trips leaving from Cleveland to Oklahoma and 233,282 trips leaving Cleveland for Lincoln. Lincoln county also overtakes Oklahoma county in terms of how many trips depart and go to other counties. 

### Vehicle Miles Traveled and Congestion

In our original model our total population was 1.4 million . In our scenario we added 250,000 additional people. This amounts to a 17.5% increase in population. In our original model, VMT was around 22.0 million VMT. In our scenario the total VMT is 40.8 million.
This amounts to an 85% increase in VMT. What we see from this is that the increase in VMT is substantially bigger than the increase in population. 

Old VMT: 22006966
New VMT: 40856275
Percent Change: 85%

Old Population: 1,425,695
New Population: 1,689,291
Percent Change: 18.4%

In addition to an increase in VMT. With current infrastructure, we would see much more congestion. The distribution of congestion for before and after our scenario are shown below. The dark purples shows where there is overlap. While both distributions peak around 1 (log VOC), this distribution shows a clear increase in links with very high levels of congestion. 
...
This scenario has tested an extreme-case, a 18.4% increase in population with no additional road network or transportation infrastructure. But a decision maker looking at this could see information such as how might traffic respond to large increases in population (it would get more congested), how would the creation of a poly-centric city impact the city-center (potentially less trips to original downtown). 

This analysis has not answered questions about the impact on smaller roads within each zone. This could be assessed using a four-step model, but is outside the scope of this project. Knowing the impact on smaller roads would be important for a decision maker considering a large population increase. 


This analysis and a four-step model generally cannot answer questions about how people would feel about new conditions. 
